import { $n as toRefs, Bt as onUpdated, Ft as onMounted, Kn as ref, U as computed, Ut as provide, V as cloneVNode, Wn as reactive, _n as watchEffect, cn as useId, gn as watch, jt as onBeforeUnmount, nt as defineComponent, pt as h, qn as shallowReactive, ut as getCurrentInstance, xt as inject, zt as onUnmounted } from "./vue.runtime.esm-bundler-dM5W6Okq.js";
//#region node_modules/vue3-carousel/dist/carousel.mjs
/**
...
var createSlideRegistry = (emit) => {
	...
	const updateSlideIndexes = (startIndex) => {
		if (startIndex !== void 0) slides.slice(startIndex).forEach((slide, offset) => {
			var _a;
			(_a = slide.exposed) === null || _a === void 0 || _a.setIndex(startIndex + offset);
		});
		else slides.forEach((slide, index) => {
			var _a;
			(_a = slide.exposed) === null || _a === void 0 || _a.setIndex(index);
		});
	};
	return {
		cleanup: () => {
...
function createCloneSlides({ slides, position, toShow }) {
	...
	if (slides.length <= 0) return clones;
	for (let i = start; i < end; i++) {
		const props = {
			index: isBefore ? i : i + slides.length,
			isClone: true,
			id: void 0,
...
function disableChildrenTabbing(node) {
	if (!node.el || !(node.el instanceof Element)) return;
	const elements = node.el.querySelectorAll(FOCUSABLE_ELEMENTS_SELECTOR);
	for (const el of elements) if (el instanceof HTMLElement && !el.hasAttribute("disabled") && el.getAttribute("aria-hidden") !== "true") el.setAttribute("tabindex", "-1");
}
...
function getTransformValues(el) {
	...
	if (values.length > 0 && !values.some(isNaN)) transformCache.set(el, values);
	return values;
}
/**
...
function invalidateTransformCache(elements) {
	if (elements) elements.forEach((el) => transformCache.delete(el));
}
function getScaleMultipliers(transformElements) {
	if (transformElements.size === 0) return {
		widthMultiplier: 1,
		heightMultiplier: 1
	};
	...
	transformElements.forEach((el) => {
		...
		if (transformArr.length === 6) {
			...
			if (scaleX !== 0) widthMultiplier /= scaleX;
			if (scaleY !== 0) heightMultiplier /= scaleY;
		}
	});
	return {
		widthMultiplier,
...
function getSnapAlignOffsetBySlideAndViewport(align, slideSize, viewportSize) {
	switch (align) {
		case "start": return 0;
		case "center":
		case "center-odd": return (viewportSize - slideSize) / 2;
		case "center-even": return viewportSize / 2 - slideSize;
		case "end": return viewportSize - slideSize;
		default: return 0;
	}
}
/**
...
function getSnapAlignOffset({ slideSize, viewportSize, align, itemsToShow }) {
	if (itemsToShow !== void 0) return getSnapAlignOffsetByItemsToShow(align, itemsToShow);
	if (slideSize !== void 0 && viewportSize !== void 0) return getSnapAlignOffsetBySlideAndViewport(align, slideSize, viewportSize);
	return 0;
}
...
function throttle(fn, ms = 0) {
	...
	function throttled(...args) {
		...
		isThrottled = true;
		if (ms > 16) timeoutId = setTimeout(() => {
			fn(...args);
			isThrottled = false;
			timeoutId = null;
		}, ms);
		else frameId = requestAnimationFrame(() => {
			fn(...args);
			isThrottled = false;
			frameId = null;
		});
...
function toCssValue(value, unit = "px") {
	...
	if (typeof value === "number" || parseFloat(value).toString() === value) return `${value}${unit}`;
	return value;
}
...
function useDrag(options) {
	...
	const handleDragStart = (event) => {
		...
		if ([
			...
			"TEXTAREA",
			"SELECT"
		].includes(targetTagName) || sliding.value) return;
		...
		(_a = options.onDragStart) === null || _a === void 0 || _a.call(options);
	};
	const handleDrag = throttle((event) => {
		...
		(_a = options.onDrag) === null || _a === void 0 || _a.call(options, {
			deltaX: dragged.x,
			deltaY: dragged.y,
			isTouch
		});
	...
	const handleDragEnd = () => {
		...
		if (!isTouch && draggedDistance > 10) window.addEventListener("click", (e) => {
			e.preventDefault();
			e.stopPropagation();
		}, {
			once: true,
		...
		(_a = options.onDragEnd) === null || _a === void 0 || _a.call(options);
		dragged.x = 0;
		dragged.y = 0;
...
function useWheel(options) {
	...
	const handleScroll = (event) => {
		...
		const threshold = typeof config.mouseWheel === "object" ? (_a = config.mouseWheel.threshold) !== null && _a !== void 0 ? _a : 10 : 10;
		...
		const isScrollingForward = (primaryDelta !== 0 ? primaryDelta : vertical.value ? deltaX : deltaY) > 0;
		(_b = options.onWheel) === null || _b === void 0 || _b.call(options, {
			deltaX,
			deltaY,
			isScrollingForward
		});
...
var Carousel = defineComponent({
	...
	props: {
		...
		breakpointMode: {
			...
			validator(value) {
				...
				if (!isValid) console.warn(`[vue3-carousel]: Invalid breakpointMode "${value}". Allowed values: ${BREAKPOINT_MODE_OPTIONS.join(", ")}`);
				return isValid;
			}
		},
		clamp: { type: Boolean },
		dir: {
			...
			validator(value, props) {
				...
				if (["ttb", "btt"].includes(normalizedDir) && (!props.height || props.height === "auto")) console.warn(`[vue3-carousel]: The dir "${value}" is not supported with height "auto".`);
				return true;
			}
		},
		enabled: {
		...
		preventExcessiveDragging: {
			...
			type: Boolean,
			validator(value, props) {
				if (value && props.wrapAround) console.warn(`[vue3-carousel]: "preventExcessiveDragging" cannot be used with wrapAround. The setting will be ignored.`);
				return true;
			}
		},
		slideEffect: {
			...
			validator(value) {
				...
				if (!isValid) console.warn(`[vue3-carousel]: Invalid slideEffect "${value}". Allowed values: ${SLIDE_EFFECTS.join(", ")}`);
				return isValid;
			}
		},
		snapAlign: {
			...
			validator(value) {
				...
				if (!isValid) console.warn(`[vue3-carousel]: Invalid snapAlign "${value}". Allowed values: ${SNAP_ALIGN_OPTIONS.join(", ")}`);
				return isValid;
			}
		},
		touchDrag: {
	...
	setup(props, { slots, emit, expose }) {
		...
		function updateBreakpointsConfig() {
			...
			const widthSource = (fallbackConfig.value.breakpointMode === "carousel" ? (_a = root.value) === null || _a === void 0 ? void 0 : _a.getBoundingClientRect().width : typeof window !== "undefined" ? window.innerWidth : 0) || 0;
			...
			breakpointsArray.some((breakpoint) => {
				if (widthSource >= breakpoint) {
					...
					if (newConfig.i18n) Object.assign(newConfig.i18n, fallbackConfig.value.i18n, props.breakpoints[breakpoint].i18n);
					return true;
				}
				return false;
			});
			Object.assign(config, fallbackConfig.value, newConfig);
			if (!isAuto.value) config.itemsToShow = getNumberInRange({
		...
		*/
		...
		function updateViewportRectSize({ widthMultiplier, heightMultiplier }) {
			var _a;
			const rect = ((_a = viewport.value) === null || _a === void 0 ? void 0 : _a.getBoundingClientRect()) || {
				width: 0,
				height: 0
			};
			viewportRect.value = {
		...
		function updateSlideSize() {
			...
			if (isAuto.value) slideSize.value = calculateAverage(slidesRect.value.map((slide) => slide[dimension.value]));
			else {
		...
		function updateSlidesData() {
			if (!config.wrapAround && slidesCount.value > 0) currentSlideIndex.value = getNumberInRange({
		...
		const ignoreAnimations = computed(() => {
			if (typeof props.ignoreAnimations === "string") return props.ignoreAnimations.split(",");
			else if (Array.isArray(props.ignoreAnimations)) return props.ignoreAnimations;
			else if (!props.ignoreAnimations) return [];
			return false;
		});
		...
		const setAnimationInterval = (event) => {
			...
			if (!(target === null || target === void 0 ? void 0 : target.contains(root.value)) || Array.isArray(ignoreAnimations.value) && ignoreAnimations.value.includes(event.animationName)) return;
		...
		onBeforeUnmount(() => {
			...
			if (typeof document !== "undefined") handleBlur();
			if (root.value) {
		...
		*/
		...
		const handleArrowKeys = throttle((event) => {
			...
			switch (event.key) {
				case "ArrowLeft":
				case "ArrowUp":
					if (isVertical.value === event.key.endsWith("Up")) if (isReversed.value) next(true);
					else prev(true);
					break;
				case "ArrowRight":
				case "ArrowDown":
					if (isVertical.value === event.key.endsWith("Down")) if (isReversed.value) prev(true);
					else next(true);
					break;
			}
		}, 200);
		...
		*/
		function initAutoplay() {
			...
			autoplayTimer = setInterval(() => {
				if (config.pauseAutoplayOnHover && isHover.value) return;
				next();
			}, config.autoplay);
		}
		...
		*/
		...
		const onDrag = ({ deltaX, deltaY, isTouch }) => {
			...
			const threshold = isTouch ? typeof config.touchDrag === "object" ? (_b = (_a = config.touchDrag) === null || _a === void 0 ? void 0 : _a.threshold) !== null && _b !== void 0 ? _b : DEFAULT_DRAG_THRESHOLD : DEFAULT_DRAG_THRESHOLD : typeof config.mouseDrag === "object" ? (_d = (_c = config.mouseDrag) === null || _c === void 0 ? void 0 : _c.threshold) !== null && _d !== void 0 ? _d : DEFAULT_DRAG_THRESHOLD : DEFAULT_DRAG_THRESHOLD;
		...
		const onWheel = ({ deltaX, deltaY, isScrollingForward }) => {
			...
			if (isScrollingForward) if (isReversed.value) prev();
			else next();
			else if (isReversed.value) next();
			else prev();
		};
		...
		function slideTo(slideIndex, skipTransition = false) {
			if (!skipTransition && isSliding.value) return;
			const targetIndex = (config.wrapAround ? mapNumberToRange : getNumberInRange)({
				val: slideIndex,
				max: maxSlideIndex.value,
				min: minSlideIndex.value
			});
			if (currentSlideIndex.value === targetIndex) return;
			...
			if (targetIndex !== slideIndex) modelWatcher.pause();
		...
		const modelWatcher = watch(() => props.modelValue, (val) => {
			if (val === currentSlideIndex.value) return;
		...
		const clonedSlidesOffset = computed(() => {
			if (!clonedSlidesCount.value.before) return 0;
			if (isAuto.value) return slidesRect.value.slice(-1 * clonedSlidesCount.value.before).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0) * -1;
		...
		const scrolledOffset = computed(() => {
			let output = 0;
			if (isAuto.value) {
				if (currentSlideIndex.value < 0) output = slidesRect.value.slice(currentSlideIndex.value).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0) * -1;
				else output = slidesRect.value.slice(0, currentSlideIndex.value).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0);
				...
				if (!config.wrapAround) scrolledSlides = getNumberInRange({
					val: scrolledSlides,
		...
		const visibleRange = computed(() => {
			...
			let maxIndex = 0;
			{
				...
				let iterations = 0;
				const maxIterations = slides.length * 2;
				if (index < 0) accumulatedSize = slidesRect.value.slice(0, index).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0) - Math.abs(scrolledOffset.value + clonedSlidesOffset.value);
				else accumulatedSize = slidesRect.value.slice(0, index).reduce((acc, slide) => acc + slide[dimension.value] + config.gap, 0) - Math.abs(scrolledOffset.value);
		...
		const trackTransform = computed(() => {
			if (config.slideEffect === "fade") return;
			...
			if (!config.wrapAround && config.preventExcessiveDragging) {
				...
				if (isAuto.value) maxSlidingValue = slidesRect.value.reduce((acc, slide) => acc + slide[dimension.value], 0);
				else maxSlidingValue = (slidesCount.value - Number(config.itemsToShow)) * effectiveSlideSize.value;
		...
		return () => {
			...
			if (!config.enabled || !output.length) return h("section", {
				ref: root,
...
var Navigation = defineComponent({
	...
	setup(props, { slots, attrs }) {
		...
		return () => {
			if (props.carousel) carousel = props.carousel;
			if (!carousel) {
			...
			return [h("button", Object.assign(Object.assign({
				type: "button",
				...
				attrs.class
			] }), (slotPrev === null || slotPrev === void 0 ? void 0 : slotPrev()) || h(Icon, { name: getPrevIcon() })), h("button", Object.assign(Object.assign({
				type: "button",
...
var Pagination = defineComponent({
	...
	props: {
		disableOnClick: { type: Boolean },
		paginateByItemsToShow: { type: Boolean },
	...
	setup(props) {
		...
		return () => {
			...
			for (let slide = isPaginated.value ? 0 : carousel.minSlide; slide <= (isPaginated.value ? pageCount.value - 1 : carousel.maxSlide); slide++) {
				const buttonLabel = i18nFormatter(carousel.config.i18n[isPaginated.value ? "ariaNavigateToPage" : "ariaNavigateToSlide"], { slideNumber: slide + 1 });
...
var Slide = defineComponent({
	...
	setup(props, { attrs, slots, expose }) {
		...
		const slideStyle = computed(() => {
			if (carousel.config.itemsToShow === "auto") return;
		...
		return () => {
			var _a, _b;
			if (!carousel.config.enabled) return (_a = slots.default) === null || _a === void 0 ? void 0 : _a.call(slots);
			return h("li", {
				...
				onFocusin: () => {
					if (carousel.viewport) carousel.viewport.scrollLeft = 0;
